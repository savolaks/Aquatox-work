...
class FoodWebInteraction:
    ...
    egestion_coeff: Optional[float] = None
...
class FoodWeb:
    ...
    def compute_rates(
        ...
        for predator_name, interactions in self._predator_index.items():
            ...
            egestion_map = _build_egestion_map(interactions)
            if not weights:
                continue
            ...
            for prey, scaled in weighted_prey:
                ...
                egestion_coeff = egestion_map.get(prey.name)
                if egestion_coeff is None:
                    assimilation_eff = getattr(predator, "assimilation_eff", self.default_assimilation)
                else:
                    assimilation_eff = 1.0 - max(0.0, min(egestion_coeff, 1.0))
                predator_gain += consumed * assimilation_eff

            if predator_gain > 0.0:
                rates[predator.name] = rates.get(predator.name, 0.0) + predator_gain
        ...
        for predator_name in names:
            ...
            for prey_name, weight in weights.items():
                ...
                for interaction in interactions:
                    if _normalize_name(interaction.prey) == _normalize_name(target_prey):
                        egestion_coeff = interaction.egestion_coeff
                        break
                if egestion_coeff is None:
                    assimilation_eff = getattr(predator, "assimilation_eff", self.default_assimilation)
...
def _build_egestion_map(
    interactions: Iterable[FoodWebInteraction],
) -> Dict[str, Optional[float]]:
    mapping: Dict[str, Optional[float]] = {}
    for interaction in interactions:
        if interaction.egestion_coeff is None:
            continue
        mapping[interaction.prey] = interaction.egestion_coeff
    return mapping
